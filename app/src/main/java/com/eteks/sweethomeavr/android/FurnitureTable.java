...
import static com.eteks.sweethome3d.model.HomePieceOfFurniture.SortableProperty.CATALOG_ID;
...
/*		DefaultTableColumnModel printableColumnModel = new DefaultTableColumnModel();
		...
		return print(g, pageFormat, pageIndex, printableColumnModel, Color.BLACK);*/return -1;
	}
...
/*	public String getClipboardCSV() {
	 ...
	 */
	private class FurnitureTableColumnModel { //extends DefaultTableColumnModel {
...
/*		private static class PanelWithInformationIcon extends JPanel {
	...
	private static class FurnitureTreeTableModel {//PJ extends AbstractTableModel implements TreeModel {
		...
		//PJ private List<TreeModelListener>                 treeModelListeners;
		...
		public FurnitureTreeTableModel(Home home) {
			...
			//PJ this.treeModelListeners = new ArrayList<TreeModelListener>();
						...
						case ADD :
							if (!expandedGroups.isEmpty()
								...
								if (insertionIndex != -1) {
									...
									//PJ fireTableRowsInserted(insertionIndex, insertionIndex);
									//PJ fireTreeModelChanged();
								}
							}
							break;
						case DELETE :
							if (furnitureFilter != null
								...
								if (deletionIndex != -1) {
									if (expandedGroups.contains(piece)) {
										...
										//PJ fireTableRowsDeleted(deletionIndex, deletionIndex);
										//PJ fireTreeModelChanged();
									}
								}
							}
		...
		public void filterAndSortFurniture() {
			...
			/*PJ if (previousRowCount != this.filteredAndSortedFurniture.size()) {
		...
		/*PJ 	public void valueForPathChanged(TreePath path, Object newValue) {
		...
		private void fireTreeModelChanged() {
			...
			//PJ for (TreeModelListener listener : this.treeModelListeners) {
			//PJ 	listener.treeStructureChanged(new TreeModelEvent(this, new TreePath(this.home)));
			//PJ }
		}*/
