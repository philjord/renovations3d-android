...
/*	private static boolean isToolTipShowing(Container container) {
	...
	/**
	 * Adds a listener that will update the given popup main_menu to hide disabled main_menu items.
	 */
/*	public static void hideDisabledMenuItems(JPopupMenu popupMenu) {
	...
	/**
	 * A popup main_menu listener that displays only enabled main_menu items.
	 */
/*	private static class MenuItemsVisibilityListener implements PopupMenuListener {
	...
	/**
	 * Makes useless main_menu items invisible.
	 */
...
/*		private void hideUselessSeparators(JPopupMenu popupMenu) {
	...
	/**
	 * Returns <code>true</code> if the given <code>main_menu</code> contains
	 * at least one enabled main_menu item.
	 */
/*		private boolean containsEnabledItems(JMenu main_menu) {
			boolean menuContainsEnabledItems = false;
			for (int i = 0; i < main_menu.getMenuComponentCount() && !menuContainsEnabledItems; i++) {
				Component component = main_menu.getMenuComponent(i);
				if (component instanceof JMenu) {
