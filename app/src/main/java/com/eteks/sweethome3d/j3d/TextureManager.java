...
public class TextureManager {
  ...
  private static class ComparableTexture {
    ...
    /**
     * Returns the pixels of the given <code>image</code>. either an int[] or a byte[]
     */
    private Object getImageBits() {
		Object imageBits = null;
      if (this.imageBits != null) {
        imageBits = this.imageBits.get();
      }
      if (imageBits == null) {
		  ImageComponent2D ic2d = (ImageComponent2D)this.texture.getImage(0);

		  if (ic2d.getImageClass() != ImageComponent.ImageClass.NIO_IMAGE_BUFFER) {
			  ...
			  imageBits = (Object)image.getRaster().getDataElements(0, 0, image.getWidth(), image.getHeight(), null);
			  this.transparent = image.getTransparency() != BufferedImage.OPAQUE;
			  if (this.transparent) {
				  this.transparent = containsTransparentPixels(imageBits);
			  }
			  this.imageBits = new WeakReference<Object>(imageBits);
		  }
		  else
		  {
			  NioImageBuffer image = ((ImageComponent2D) this.texture.getImage(0)).getNioImage();
			  imageBits =  ((ByteBuffer) image.getDataBuffer()).array();
    ...
    private int getImageBitsHashCode() {
      if (this.imageBitsHashCode == null) {
		  Object thisBits = getImageBits();
		  if( thisBits instanceof int[]  )
			  this.imageBitsHashCode = Arrays.hashCode((int[])thisBits);
		  else
			  this.imageBitsHashCode = Arrays.hashCode((byte[])thisBits);
      }
     ...
     */
    private boolean containsTransparentPixels(Object imageBits) {
      boolean transparentPixel = false;
		if( imageBits instanceof int[])
		{
			for (int argb : (int[]) imageBits)
			{
		...
		{
			return false;
			//PJ the below is incorrect, but false doesn't appear to cause any trouble?
		/*	byte[] ba = (byte[]) imageBits;
    ...
    public boolean equalsImage(ComparableTexture comparableTexture) {
      if (this == comparableTexture) {
        ...
        return true;
		  ...
		  Object thisBits = getImageBits();
		  Object otherBits = comparableTexture.getImageBits();
		  if( thisBits instanceof int[] && otherBits instanceof int[])
        return Arrays.equals((int[])thisBits, (int[])otherBits);
		  else  if( thisBits instanceof byte[] && otherBits instanceof byte[])
			  return Arrays.equals((byte[])thisBits, (byte[])otherBits);
      }
      return false;
    }
  }
